...
/************************************** POST /users/:username/words/:id */

describe("POST /users/:username/words/:id", function () {
  test("works for admin", async function () {
    const resp = await request(app)
  ...
  test("unauth for anon", async function () {
    const resp = await request(app).post(`/users/u1/words/${testWordIds[1]}`);
    expect(resp.statusCode).toEqual(401);
  });

  test("not found for no such username", async function () {
    const resp = await request(app)
      .post(`/users/nope/words/${testWordIds[1]}`)
      .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
  });
  ...
  test("not found for no such word", async function () {
    const resp = await request(app)
      .post(`/users/u1/words/0`)
      .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
  });

  test("bad request invalid word id", async function () {
    const resp = await request(app)
      .post(`/users/u1/words/0`)
      .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
